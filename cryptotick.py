...
def makeSpark(allprices):
    ...
    for key in allprices.keys():   
        ...
        plt.savefig(os.path.join(picdir, key+'spark.png'), dpi=70)
...
def updateDisplay(image,config,allprices, volumes):
    ...
    heightincrement=int(283*scaling)
    index=0
    for key in allprices.keys():
    ...
    #   Check for token image, if there isn't one, get on off coingecko, resize it and pop it on a white background
        ...
        image.paste(sparkpng, (1000,height+150))
        image.paste(tokenimage, (50,height+100))
        ...
        if len(text)>7:
            ...
            pricefontsize=80
        _place_text(image, text, x_offset=-240, y_offset=height-480,fontsize=pricefontsize,fontstring="PressStart2P-Regular")
        vol = human_format(volumes[key+"volume"])
        text=pricechange + " vol:" + symbolstring + vol
        _place_text(image, text, x_offset=-240, y_offset=height-400,fontsize=35,fontstring="PressStart2P-Regular")
    ...
    _place_text(image, "Updated: "+text+". "+str(days_ago)+" day data", x_offset=-60, y_offset=-425,fontsize=36,fontstring="PressStart2P-Regular")
...
def display_startup(display):
    ...
    img = Image.new("RGB", (1872, 1404), color = (255, 255, 255) )
    _place_text(img, 'WiFi: '+ ssid, x_offset=-50, y_offset=-80,fontsize=50,fontstring="PressStart2P-Regular")
    _place_text(img, 'IP: '+ get_ip(), x_offset=-75, y_offset=0,fontsize=50,fontstring="PressStart2P-Regular")
    # update display
    img=img.rotate(180, expand=True)
    display.frame_buf.paste(img, paste_coords)
    display.draw_full(constants.DisplayModes.GC16)
