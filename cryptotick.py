...
def updateDisplay(image,config,allprices, volumes):
 ...
 #       draw.text((110,90),str(days_ago)+"day : "+pricechange,font =font_date,fill = 0)
        ...
        _place_text(image, text, x_offset=-190, y_offset=height-400,fontsize=130,fontstring="JosefinSans-Light")
        vol = human_format(volumes[key+"volume"])
        text=pricechange + " vol:" + symbolstring + vol
        _place_text(image, text, x_offset=-190, y_offset=height-315,fontsize=50,fontstring="JosefinSans-Light")
