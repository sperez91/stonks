...
def updateDisplay(image,config,allprices, volumes):
 ...
 #       draw.text((110,90),str(days_ago)+"day : "+pricechange,font =font_date,fill = 0)
        ...
        image.paste(sparkpng, (690,height+40))
        image.paste(tokenimage, (80,height+30))

        text=symbolstring+pricenowstring
        _place_text(image, text, x_offset=-210, y_offset=height-400,fontsize=130,fontstring="JosefinSans-Light")
        vol = human_format(volumes[key+"volume"])
        text=pricechange + " vol:" + symbolstring + vol
        _place_text(image, text, x_offset=-210, y_offset=height-315,fontsize=50,fontstring="JosefinSans-Light")
